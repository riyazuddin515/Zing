 ...
 */
class MessageSwipeController @Inject constructor(
    @ApplicationContext private val context: Context
) : ItemTouchHelper.Callback() {

    private var onSwipeControllerAction: ((Int) -> Unit)? = null
    fun setOnSwipeControllerAction(listener: (Int) -> Unit) {
        onSwipeControllerAction = listener
    }
        ...
        recyclerView.setOnTouchListener { _, event ->
            ...
            if (swipeBack) {
                if (abs(mView.translationX) >= this@MessageSwipeController.convertTodp(100)) {
                    val a = viewHolder.absoluteAdapterPosition
                    onSwipeControllerAction?.let {
                        it(a)
                    }
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        shareRound.colorFilter =
            PorterDuffColorFilter(
                ContextCompat.getColor(context, R.color.replyBg),
